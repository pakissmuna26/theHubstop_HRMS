...
<?php
	...
	for($i=0; $User = $result->fetch(); $i++){
		...
		while ($payroll_period = mysqli_fetch_array($payroll_periods)) {
			$payroll_period_details = "<span style='color:gray;'>{$payroll_period['payroll_period_code']}</span><br>
			...
			if($payroll_period_benefits_deduction['benefits_deduction_category']=="Benefits"){
				...
				while ($Deduction = mysqli_fetch_array($Deductions)) {
					if($Deduction['deduction_category_is_percentage'] == "Yes")
						$deduction_details="<b>{$Deduction['deduction_category_title']}</b><br>
						- Company Share: {$Deduction['deduction_category_company_share']}% <br>
						- Personnel Share: {$Deduction['deduction_category_personnel_share']}%";
					else if($Deduction['deduction_category_is_percentage'] == "No")
						$deduction_details="<b>{$Deduction['deduction_category_title']}</b><br>
						- Company Share: PHP ".addComma($Deduction['deduction_category_company_share'])." <br>
						- Personnel Share: PHP ".addComma($Deduction['deduction_category_personnel_share']);
					break;
				}
		...
		$jsonArrayItem['payroll_period_details'] = $payroll_period_details;
