    ...
    <div class="layout-page">
      <!-- Navbar -->
        ...
        <script type="text/javascript">
          $(".page_name").text("Manual Attendance Management");          
        </script>
      <!-- / Navbar -->
            ...
            <div class="col-lg-12">
              <div class="card">
                <div class="card-body">
                  <div class="row">
                    <div class="col-lg-3">
                      <button type="button" id="btnResetPassword" class="btn btn-outline-success" data-bs-toggle='modal' data-bs-target='#modalManualAttendanceManagement' onclick="btnAddNewData()">Add New Data</button>
                    </div>
                  </div>
                  <div class="row">
...
function ListOfData(){
  let obj={"attendance_category":"Manual"};
    ...
    "columnDefs": [{
      ...
      "render": function ( data, type, row ) {
        ...
        if(row.time_in_valid){
          ...
          }else if(row.attendance_status == "Approved"){
            button_change_status = "<a class='dropdown-item' data-bs-toggle='modal' data-bs-target='#modalChangeStatus' onclick='btnChangeStatus("+row.attendance_id+", \"Denied\", \"Deny\")'><i class='bx bx-refresh'></i> Deny Attendance</a>";
          }else if(row.attendance_status == "Denied"){
            button_change_status = "<a class='dropdown-item' data-bs-toggle='modal' data-bs-target='#modalChangeStatus' onclick='btnChangeStatus("+row.attendance_id+", \"Approved\", \"Approve\")'><i class='bx bx-refresh'></i> Approve Attendance</a>";
          }
        }else{
          button_update_details = "<a class='dropdown-item' data-bs-toggle='modal' data-bs-target='#modalManualAttendanceManagement' onclick='btnUpdateDetails("+row.attendance_id+")'><i class='bx bx-edit'></i> Update Details</a>";
...
function CheckDuplicateEntry(){
  ...
  let attendance_type = $("#select_attendance_type").val();
  ...
  $.ajax({url:'tbl_attendance/json_attendance.php',
    ...
    success:function(data){
      let exist = false;
      for(let index = 0; index < data.length; index++){
        if(data[index].attendance_category == attendance_category &&
            data[index].attendance_type == attendance_type && 
            data[index].attendance_date_in == attendance_date_in &&
            data[index].attendance_time_in == attendance_time_in && 
            data[index].attendance_date_out == attendance_date_out && 
            data[index].attendance_time_out == attendance_time_out && 
            data[index].attendance_requested_by == attendance_requested_by){
          exist = true;
          break;
        }
      }
      ...
      if(exist){
        ShowToast("bg-warning", "Warning", "Attendance is already exist.");
      }else{
        SaveAttendance();        
...
function btnUpdateDetails(attendance_id) {
  ...
  $.ajax({url:'tbl_attendance/json_attendance.php',
    ...
    success:function(data){
      for(let index = 0; index < data.length; index++){
        if(data[index].attendance_id == attendance_id){        
          $("#select_attendance_type").val(data[index].attendance_type);
      ...
      <div class="modal-body">
        <div class="row">
          <div class="col-lg-12">
            ...
            <select class="form-control fields" id="select_attendance_type">
